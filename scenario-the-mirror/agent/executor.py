...
def generate_postmortem(incident_id, attacker_ip, detection, osint_data, audit):
    ...
    # CTF Enhancement: GitHub Issues + Slack Integration
    try:
        from agent.github_integration import GitHubIssueManager
        from agent.slack_integration import SlackNotifier

        # Initialize GitHub integration
        github_mgr = GitHubIssueManager()

        # Create GitHub issue
        github_issue = github_mgr.create_incident_issue(
            incident_id=incident_id,
            attacker_ip=attacker_ip,
            detection=detection,
            ...
            timeline=incident_data["timeline"],
            recommendations=incident_data["recommendations"],
        )

        if github_issue:
            logger.info(
                f"Created GitHub issue #{github_issue.number}: {github_issue.html_url}"
            )
            ...
            # Log GitHub integration to audit
            audit.record(
                incident_id=incident_id,
                action_id="create-github-issue",
                action_name="Create GitHub issue for incident",
                tier=None,
                parameters={
                ...
                justification={
                    ...
                    "evidence_refs": [],
                    "playbook_rule": None,
                    "reasoning": "GitHub issue created with OSINT dossier and audit trail",
                },
                context={
                    "attacker_ip": attacker_ip,
        ...
        logger.info(
            "GitHub/Slack integration modules not available (PyGithub, slack-sdk not installed)"
        )
    except Exception as e:
        logger.error(f"Failed to create GitHub issue or send Slack notification: {e}")
